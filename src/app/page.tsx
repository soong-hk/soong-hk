  ...
  return (
    <>
      ...
      <div className="min-h-screen" style={{ background: "var(--color-cream)" }}>
        <Header />
        <main>
          <section className="relative px-4 py-12 sm:py-20 text-center overflow-hidden" style={{ background: "linear-gradient(160deg, var(--color-warm) 0%, var(--color-cream) 60%)" }}>
          ...
          <section className="mx-auto max-w-4xl px-4 py-6">
            <p className="mb-3 text-sm font-medium text-center" style={{ color: "var(--color-text-muted)" }}>依據你剛做完的事，挑選最適合的舒緩動作</p>
            <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
              {SCENARIO_TAGS.map((tag) => (
                <button key={tag.label} onClick={() => handleScenarioClick(tag)} className="flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-medium transition-all hover:scale-105 active:scale-95 hover:shadow-md" style={{ background: activeTag === tag.label ? "var(--color-sage)" : "white", color: activeTag === tag.label ? "white" : "var(--color-text)", border: `1px solid ${activeTag === tag.label ? "var(--color-sage)" : "var(--color-border)"}` }}>
          ...
          <section className="mx-auto max-w-4xl px-4 pb-8">
            {hasSearched && !isLoading && results.length > 0 && (
              <div className="mb-5">
                ...
                <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>共找到 {results.length} 個影片 · <span style={{ color: "var(--color-sage-dark)" }}>🤖 AI 篩選結果</span></p>
              </div>
            )}
            {isLoading && (
